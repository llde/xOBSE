...
public:
    struct Data{
		...
		bool                                temporary;  //This mark a IrefData that has no ExtraDataList and or ExtraData
		Data(TESForm* t, ExtraContainerChanges::EntryData* en, ExtraDataList* ex) : type(t), entry(en), xData(ex), count(-1),temporary(false) { }
		Data(TESForm* t, ExtraContainerChanges::EntryData* en, SInt32 count) : type(t), entry(en), xData(nullptr), count(count), temporary(true) { }  //if EntryData* NULL, it's an object in ther base container.
		Data(const Data& rhs) : type(rhs.type), entry(rhs.entry), xData(rhs.xData), count(rhs.count), temporary(rhs.temporary) { }
		Data() : type(NULL), entry(NULL), xData(NULL),count(-1),temporary(false) { }
    };
